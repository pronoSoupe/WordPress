...
class wps_shipping {
	...
	function get_shipping_cost( $nb_of_items, $total_cart, $total_shipping_cost, $total_weight, $selected_method = '' ) {
		...
		if( !empty($selected_method) || isset( $_SESSION['shipping_address'] ) ) {
			if( !empty($selected_method) ) {
				$chosen_shipping_mode = $selected_method;
			} else {
				$ship_mod = new wps_shipping_mode_ctr();
				$chosen_shipping_mode = $ship_mod->get_shipping_mode_for_address( $_SESSION['shipping_address'] );
				$chosen_shipping_mode = $chosen_shipping_mode['modes'];
				reset( $chosen_shipping_mode );
				$chosen_shipping_mode = key( $chosen_shipping_mode );
			}
		}
		else {
