...
// 		$products_post = $wpdb->get_results( $query );
	...
	*/
	public static function create_wpshop_products_type() {
		...
		( empty( $options['wpshop_catalog_product_slug'] ) || $options['wpshop_catalog_product_slug'] == '/' ) ? $slug = false : $slug['slug'] = $options['wpshop_catalog_product_slug'] . ( !empty( $options['wpshop_catalog_product_slug_with_category'] ) ? '/%' . WPSHOP_NEWTYPE_IDENTIFIER_CATEGORIES . '%' : '' );
	...
	*/
	...
	function save_product_custom_informations( $post_id , $data_to_save = array() ) {
		...
		if ( !empty($data_to_save['post_ID']) && (get_post_type($data_to_save['post_ID']) == WPSHOP_NEWTYPE_IDENTIFIER_PRODUCT) ) {
			...
			add_post_meta( $post_id, '_wpshop_variation_defining', array( 'options' => array( 'priority' => array( 'combined' ), 'price_behaviour' => array( 'replacement' ), 'price_display' => array( 'text_from' => 'on', 'lower_price' => 'on' ) ) ), true );
		}
